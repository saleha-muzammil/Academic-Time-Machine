...
int main(void)
{
		...
		for (i = 0; buff[i] != '\0'; i++)
		{
			if(buff[i] != 32) // word not end
			{
				wordArray[j] = buff[i];
				j++;
				if( buff[i] == 65 || buff[i] == 97 ||
					buff[i] == 69 || buff[i] == 101 ||
					buff[i] == 73 || buff[i] == 105 ||
					buff[i] == 79 || buff[i] == 111 ||
					buff[i] == 85 || buff[i] == 117)
				{
					vowels++;
				}
			}
        ...
        printf("Server Message with non-vowels inverted: %s\n",buff);
