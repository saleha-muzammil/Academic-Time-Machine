...
select [Questions].questionId,[Questions].question,[Questions].userId,[Users].dob
from [Questions] join [Users] on [Questions].userId=[Users].userId


---b---
go
create view v1(noofqs,category)as 
...
go
create procedure p1 
@input_category1 varchar(40),
@input_userid1 nvarchar(50),
...
begin
	...
	if (exists(select userId
           from [Answers]
		   where userId=@input_userid1))

	begin
		select @output_param1=isnull(count(*),0)
		from [Answers] join [Questions] on [Answers].questionId=[Questions].questionId
		where [Answers].userId=@input_userid1 and [Questions].category=@input_category1
...
declare @xx1 int
exec p1 @input_category1='Programing',
@input_userid1='john',
@output_param1=@xx1 OUTPUT

select @xx1


--select [Answers].userId,[Questions].category
--from [Answers] join [Questions] on [Answers].questionId=[Questions].questionId
...
select top (1) y.category,y.countx,y.questionId,y.userId
from(

	select t1.questionId,t1.countx,[Questions].userId,[Questions].category
	from (select top (@input1) count(*)as countx,questionId 
	from [Answers]
	group by [Answers].questionId
	order by countx desc) as t1 join [Questions] on t1.questionId=[Questions].questionId
...
--select t1.questionId,t1.countx,[Questions].userId,[Questions].category
--from (select top (3) count(*)as countx,questionId 
--from [Answers]
--group by [Answers].questionId
--order by countx desc) as t1 join [Questions] on t1.questionId=[Questions].questionId
...
where u2.userId in (select follower
             from [Followers]
			 where followee=u1.userId)
	  and
	   u1.userId in (select follower
             from [Followers]
			 where followee=u2.userId)
...
not exists( select category
			...
			from Questions 
			where [Questions].userId= u.userId
			)
