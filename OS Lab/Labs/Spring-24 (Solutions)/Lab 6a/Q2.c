...
int main(){
    ...
    //writing file
    int fd1=open(fName1, O_WRONLY | O_CREAT);
    if (fd1==-1){
        printf("File for write not opened correctly!\n");
        return 2;
    }
    setvbuf(stdout, NULL, _IONBF, 0); //this sets stdout to line buffered so that the data is printed to the file before the stdout is reset to the original value i.e. prints to the terminal
